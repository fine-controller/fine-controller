...
namespace Systems.KubernetesSystem.Impl
{
	internal class KindKubernetesSystemImpl : KubernetesSystemImpl
	{
		...
		{
			await ProcessUtil.ExecuteAsync(_kindExecutableFile, new[] { "create", "cluster", "--name", Constants.FineController, "--config", CLUSTER_CONFIGURATION_YAML_FILE }, _appCancellationToken.Token);
		}
